...
_DAYS_IN_MONTH = [None, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28]
...
def _ord2ymd(n):
    ...
    # Calculate preceeding years.  Krynn years are perfectly regular.
    year, n = divmod(n, 336)
    # Calculate preceeding months.  Krynn months are prefectly regular.
    month, n = divmod(n, 28)
...
def _rising_falling(minutes):
    result = ('falling,', minutes)
    if minutes == 0:
        result = ('chiming', '')
    elif minutes == 30:
        result = (', mid-watch', '')
    if minutes > 30:
        result = ('rising,', str(30 - (minutes % 30)))
    return result

def _build_struct_time(y, m, d, hh, mm, ss):
    wday = (_ymd2ord(y, m, d) + 6) % 7
    dnum = _days_before_month(y, m) + d
    return _time.struct_time((y, m, d, hh, mm, ss, wday, dnum, False))

def _format_time(hh, mm, ss, us=None):
...
_ORD1970 = _ymd2ord(1, 1, 1) # base ordinal for Cataclysm epoch
...
class tmxxx:
    ...
    def ctime(self):
        ...
        inflection, mincount = _rising_falling(self.minute)
        return "AC {} {}, {} {}  {} {} {}".format(
            self.year,
            _DAYNAMES[weekday],
            _MONTHNAMES[self.month],
            self.day,
            _HOURNAMES[self.hour],
            inflection,
            mincount)
    def ktime(self):
        "Return ktime() style string."
        weekday = self.toordinal() % 7 or 7
        inflection, mincount = _rising_falling(self.minute)
        return "AC {} {} {}, {} {} {}".format(
            self.year,
            ...
            mincount)
    def kdatestr(self):
        "Return ktime() style string for date only."
        weekday = self.toordinal() % 7 or 7
        return "AC {} {} {}".format(
            self.year,
            _MONTHNAMES[self.month],
...
class timedelta(object):
    ...
    def __repr__(self):
        if self.__microseconds:
            return "%s(%d, %d, %d)" % ('krynndatetime.' + self.__class__.__name__,
                                       self.__days,
                                       self.__seconds,
                                       self.__microseconds)
        if self.__seconds:
            return "%s(%d, %d)" % ('krynndatetime.' + self.__class__.__name__,
                                   self.__days,
                                   self.__seconds)
        return "%s(%d)" % ('krynndatetime.' + self.__class__.__name__, self.__days)
...
class date(object):
    ...
    def ktime(self):
        "Format a la ktime()."
        return tmxxx(self.__year, self.__month, self.__day).ktime()

    def kdatestr(self):
        "Format a la kdatestr()."
        return tmxxx(self.__year, self.__month, self.__day).kdatestr()
    ...
    def isoformat(self):
        ...
        """
        return "%03d-%02d-%02d" % (self.__year, self.__month, self.__day)
    ...
    day = property(lambda self: self.__day, doc="day (1-28)")
...
class time(object):
    ...
    def __repr__(self):
        ...
        if self.__microsecond != 0:
            ...
            s = ""
        s= "%s(%d, %d%s)" % ('krynndatetime.' + self.__class__.__name__,
...
class datetime(date):
    ...
    def __repr__(self):
        ...
        while L[-1] == 0:
            del L[-1]
        s = ", ".join(map(str, L))
        s = "%s(%s)" % ('krynndatetime.' + self.__class__.__name__, s)
        if self._tzinfo is not None:
            assert s[-1:] == ")"
            s = s[:-1] + ", tzinfo=%r" % self._tzinfo + ")"
        return s
...
datetime.max = datetime(9999, 12, 28, 23, 59, 59, 999999)
